...
# NUEVA SECCIÓN: Botón para resetear la fila actual
if st.button("Resetear fila (Revertir cambios)"):
    ...
    ref_sat = current_row["Ref. Saturno"]
    nombre = current_row["Nombre producto"].strip().lower()
    original_match = original_df[(original_df["Ref. Saturno"] == ref_sat) & (original_df["Nombre producto"].str.lower() == nombre)]
...
# NUEVA SECCIÓN: Si se ingresó Fecha Pedida, multiselect para pedir el lote completo.
